...
async function main(): Promise<void> {
  ...
  // Fast-path: Goatmez adapter namespace inside Code Engine primary runtime.
  if (args[0] === 'goatmez') {
    const {
      runGoatmezCli
    } = await import('../goatmez/cli.js');
    await runGoatmezCli(args.slice(1));
    return;
  }
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Code Engine)`);
    return;
  }
