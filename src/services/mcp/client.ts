...
import * as MCPClientAuth from '@modelcontextprotocol/sdk/client/auth.js'
...
// mcpConfigs object ref carried over — main.tsx creates fresh config objects each call.
...
async function callMCPTool({
  ...
  try {
    ...
    if (e instanceof Error) {
      ...
      if (errorCode === 401 || e instanceof AuthRequiredError) {
        logMCPDebug(
          name,
          `Tool call returned 401 Authentication required - token may have expired`,
        )
