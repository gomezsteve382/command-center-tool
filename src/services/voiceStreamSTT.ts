...
// OAuth credentials as Code Engine.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Code Engine — available when the
...
export async function connectVoiceStream(
  ...
  // non-browser clients (anthropics/code-engine#34094). Same private-api
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/code-engine#40510).
