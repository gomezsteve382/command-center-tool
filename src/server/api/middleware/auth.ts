...
export function requireAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  if (AUTH_PROVIDER === "token") {
    ...
    if (!token || token !== AUTH_TOKEN) {
      res.status(401).json({
        error: { code: "AUTH_REQUIRED", message: "Invalid or missing auth token" },
      });
      return;
    }
    (req as AuthenticatedRequest).user = { id: "default", isAdmin: true };
  ...
  if (AUTH_PROVIDER === "apikey") {
    ...
    if (!apiKey) {
      res.status(401).json({
        error: { code: "AUTH_REQUIRED", message: "Anthropic API key required" },
      });
      return;
    }
  ...
  res.status(401).json({ error: { code: "AUTH_REQUIRED", message: "Unauthenticated" } });
}
