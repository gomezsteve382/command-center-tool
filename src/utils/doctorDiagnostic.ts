...
> {
  ...
  const packagesToCheck = ['@srtlab/code-engine']
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@srtlab/code-engine') {
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @srtlab/code-engine'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@srtlab/code-engine'
        ) {
