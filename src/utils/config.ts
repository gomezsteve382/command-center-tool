...
export type GlobalConfig = {
  ...
  firstStartTime?: string // ISO timestamp when Code Engine was first started on this machine
  ...
  // Code Engine usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first Code Engine OAuth token
  ...
  // Code Engine hint protocol state (<code-engine-hint /> tags from CLIs/SDKs).
  ...
  try {
    ...
    release = lockfile.lockSync(file, {
      ...
      onCompromised: err => {
        ...
        // missed (e.g. after a 10s event-loop stall) is recoverable.
