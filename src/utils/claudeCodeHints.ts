/**
 * Code Engine hints protocol.
 *
 * CLIs and SDKs running under Code Engine can emit a self-closing
 * `<code-engine-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/code-engine-hints.md for the vendor-facing spec.
 */
 ...
 */
const HINT_TAG_RE = /^[ \t]*<code-engine-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
...
export function extractClaudeCodeHints(
  ...
  if (!output.includes('<code-engine-hint')) {
