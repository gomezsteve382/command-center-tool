/**
 * Marketplace manager for Code Engine plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Code Engine bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Code Engine version. Run 'claude marketplace remove ${name}' and ` +
  ...
  try {
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      try {
        ...
        const reason =
          name === 'code-engine-plugins'
            ? `We've deprecated "code-engine-plugins" in favor of "claude-plugins-official".`
