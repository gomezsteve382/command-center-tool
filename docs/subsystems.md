...
> Detailed documentation of Code Engine's major subsystems.
...
The bridge is a bidirectional communication layer connecting Code Engine's CLI with IDE extensions (VS Code, JetBrains). It allows the CLI to run as a backend for IDE-based interfaces.
...
Code Engine acts as both an **MCP client** (consuming tools/resources from MCP servers) and can run as an **MCP server** (exposing its own tools via `src/entrypoints/mcp.ts`).
...
When launched via `src/entrypoints/mcp.ts`, Code Engine exposes its own tools and resources via the MCP protocol, allowing other AI agents to use Code Engine as a tool server.
...
Code Engine supports installable plugins that can extend its capabilities.
...
| Built-in plugins | `src/plugins/builtinPlugins.ts` | Plugins that ship with Code Engine |
...
| Component | Location | Purpose |
|-----------|----------|---------|
| Bundled skills | `src/skills/bundled/` | Skills that ship with Code Engine |
...
| `claudeApi` | Direct SRT Lab API interaction |
...
Code Engine's persistent memory system, based on `CLAUDE.md` files.
...
|---------|------|---------|
| **API** | `api/` | SRT Lab SDK client, file uploads, bootstrap |
