...
> Deep-dive into how Code Engine is structured internally.

---

## High-Level Overview

Code Engine is a terminal-native AI coding assistant built as a single-binary CLI. The architecture follows a pipeline model:
...
| `mcp.ts` | MCP server mode entrypoint (Code Engine as an MCP server) |
| `sdk/` | Agent SDK — programmatic API for embedding Code Engine |
...
The heart of Code Engine. Handles:

- **Streaming responses** from the SRT Lab API
...
Code Engine uses a **React context + custom store** pattern:
...
Code Engine runs on [Bun](https://bun.sh) (not Node.js). Key implications:
...
Code Engine uses a **single-threaded event loop** (Bun/Node.js model) with:
