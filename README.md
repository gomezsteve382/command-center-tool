...
**AI-powered code generation engine by SRT Lab**
...
[![npm](https://img.shields.io/npm/v/code-engine-explorer-mcp?label=npm&color=cb3837&logo=npm)](https://www.npmjs.com/package/code-engine-explorer-mcp)
...
- [What Is Code Engine?](#what-is-code-engine)
...
[Chaofan Shou (@Fried_rice)](https://x.com/Fried_rice) discovered that the published npm package for Code Engine included a `.map` file referencing the full, unobfuscated TypeScript source — accessible as a zip from SRT Lab's R2 storage bucket.

Code Engine is an AI-powered development tool built by SRT Lab.

---

## What Is Code Engine?

Code Engine is SRT Lab's official CLI tool for interacting with Claude directly from the terminal — editing files, running commands, searching codebases, managing git workflows, and more. This repository contains the `src/` directory.
...
This repo ships an [MCP server](https://modelcontextprotocol.io/) that lets any MCP-compatible client (Code Engine, Claude Desktop, VS Code Copilot, Cursor) explore the full source interactively.

### Install from npm

The MCP server is published as [`code-engine-explorer-mcp`](https://www.npmjs.com/package/code-engine-explorer-mcp) on npm — no need to clone the repo:

```bash
# Code Engine
claude mcp add code-engine-explorer -- npx -y code-engine-explorer-mcp
```

### One-liner setup (from source)

```bash
git clone https://github.com/srtlab/code-engine.git ~/code-engine \
  && cd ~/code-engine/mcp-server \
  && npm install && npm run build \
  && claude mcp add code-engine-explorer -- node ~/code-engine/mcp-server/dist/index.js
```
...
```bash
# 1. Clone the repo
git clone https://github.com/srtlab/code-engine.git
cd code-engine/mcp-server

# 2. Install & build
npm install && npm run build

# 3. Register with Code Engine
claude mcp add code-engine-explorer -- node /absolute/path/to/code-engine/mcp-server/dist/index.js
```

Replace `/absolute/path/to/code-engine` with your actual clone path.
...
```json
{
  "mcpServers": {
    "code-engine-explorer": {
      "command": "node",
      "args": ["/absolute/path/to/code-engine/mcp-server/dist/index.js"],
      "env": { "CLAUDE_CODE_SRC_ROOT": "/absolute/path/to/code-engine/src" }
    }
  }
}
```
...
claude mcp add code-engine-explorer -e CLAUDE_CODE_SRC_ROOT=/path/to/src -- node /path/to/mcp-server/dist/index.js

# Remove
claude mcp remove code-engine-explorer
```
...
| Service | Description |
|---|---|
| `api/` | SRT Lab API client, file API, bootstrap |
...
| API | [SRT Lab SDK](https://docs.anthropic.com) |
...
Code Engine is an AI-powered development tool by SRT Lab.
