# Code Engine Explorer — MCP Server

A standalone [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets any MCP-compatible client explore the Code Engine source code. Supports **STDIO**, **Streamable HTTP**, and **SSE** transports.

## What It Does

Exposes 8 tools, 3 resources, and 5 prompts for navigating the ~1,900-file, 512K+ line Code Engine codebase:

### Transports

| Transport | Endpoint | Best For |
|-----------|----------|----------|
| **STDIO** | `node dist/index.js` | Claude Desktop, local Code Engine, VS Code |
...
| URI | Description |
|-----|-------------|
| `code-engine://architecture` | README / architecture overview |
| `code-engine://tools` | Tool registry (JSON) |
| `code-engine://commands` | Command registry (JSON) |
| `code-engine://source/{path}` | Any source file (template) |
...
| `architecture_overview` | Guided tour of the full Code Engine architecture |
...
```json
{
  "mcpServers": {
    "code-engine-explorer": {
      ...
      "env": {
        "CLAUDE_CODE_SRC_ROOT": "/absolute/path/to/code-engine/src"
      }
    }
  }
}
...
|----------|---------|-------------|
| `CLAUDE_CODE_SRC_ROOT` | `../src` (relative to dist/) | Path to the Code Engine `src/` directory |
...
```bash
# From repo root
docker build -f mcp-server/Dockerfile -t code-engine-mcp .
docker run -p 3000:3000 -e MCP_API_KEY=your-secret code-engine-mcp
```
...
| `architecture_overview` | Guided tour of the entire Code Engine architecture |
...
Registry name: `io.github.srtlab/code-engine-explorer-mcp`
