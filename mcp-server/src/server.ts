 ...
 *
 * Exposes tools and resources for exploring the Code Engine source code.
...
export function createServer(): Server {
  const server = new Server(
    { name: "code-engine-explorer", version: "1.1.0" },
    {
      capabilities: {
        tools: {},
        resources: {},
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async () => ({
    resources: [
      {
        uri: "code-engine://architecture",
        name: "Architecture Overview",
        description:
          "High-level overview of the Code Engine source architecture",
        mimeType: "text/markdown",
      },
      {
        uri: "code-engine://tools",
        name: "Tool Registry",
        description: "List of all agent tools with their files",
        mimeType: "application/json",
      },
      {
        uri: "code-engine://commands",
  ...
  server.setRequestHandler(
    ...
    async () => ({
      resourceTemplates: [
        {
          uriTemplate: "code-engine://source/{path}",
          name: "Source file",
          description:
            "Read a source file from the Code Engine src/ directory",
          mimeType: "text/plain",
        },
      ],
    })
  ...
  server.setRequestHandler(
    ...
    async (request: { params: { uri: string } }) => {
      ...
      if (uri === "code-engine://architecture") {
      ...
      if (uri.startsWith("code-engine://source/")) {
        const relPath = uri.slice("code-engine://source/".length);
    ...
    tools: [
      {
        name: "list_tools",
        description:
          "List all Code Engine agent tools (BashTool, FileReadTool, etc.) with their source files.",
        inputSchema: { type: "object" as const, properties: {} },
      },
      {
        name: "list_commands",
        description:
          "List all Code Engine slash commands (/commit, /review, /mcp, etc.) with their source files.",
        inputSchema: { type: "object" as const, properties: {} },
      },
      {
        name: "get_tool_source",
        description:
          "Read the full source code of a specific Code Engine tool implementation.",
        inputSchema: {
      ...
      },
      {
        ...
        description:
          "Read the source code of a specific Code Engine slash command.",
        inputSchema: {
          type: "object" as const,
          properties: {
            commandName: {
      ...
      },
      {
        ...
        description:
          "Read any source file from the Code Engine src/ directory by relative path.",
        inputSchema: {
          type: "object" as const,
          properties: {
            path: {
      ...
      },
      {
        name: "search_source",
        description:
          "Search for a regex pattern across the Code Engine source. Returns matching lines with paths and line numbers.",
        inputSchema: {
          type: "object" as const,
          properties: {
            pattern: {
      ...
      },
      {
        ...
        description:
          "Get a high-level architecture overview of Code Engine.",
        inputSchema: { type: "object" as const, properties: {} },
      },
    ],
  }));
    ...
    async (request: {
      ...
      switch (name) {
        ...
        case "get_architecture": {
          ...
          const overview = `# Code Engine Architecture Overview
    ...
    prompts: [
      {
        name: "explain_tool",
        description:
          "Explain how a specific Code Engine tool works, including its input schema, permissions, and execution flow.",
        arguments: [
          {
            name: "toolName",
      ...
      },
      {
        ...
        description: "Explain how a specific Code Engine slash command works.",
        arguments: [
          {
            name: "commandName",
      ...
      },
      {
        ...
        description:
          "Get a guided tour of the Code Engine architecture with explanations of each subsystem.",
      },
      {
        name: "how_does_it_work",
        description:
          "Explain how a specific feature or subsystem of Code Engine works.",
        arguments: [
          {
            name: "feature",
            description:
      ...
      },
      {
        name: "compare_tools",
        description:
          "Compare two Code Engine tools side by side — purpose, inputs, permissions, implementation.",
        arguments: [
    ...
    async (request: {
      ...
      switch (name) {
            ...
            messages: [
              {
                ...
                content: {
                  type: "text" as const,
                  text: `Analyze and explain this Code Engine slash command. Cover:\n1. Purpose\n2. Type (prompt vs action)\n3. Allowed Tools\n4. Arguments\n5. Implementation\n\nFiles: ${fileList}\n\nSource:\n\`\`\`typescript\n${source}\n\`\`\``,
                },
              },
            ],
          };
        ...
        case "architecture_overview": {
          ...
          const commands = await getCommandList();
          return {
            description: "Architecture overview of Code Engine",
            messages: [
              {
                role: "user" as const,
                content: {
                  type: "text" as const,
                  text: `Give a comprehensive guided tour of the Code Engine architecture.\n\n## README\n${readme}\n\n## src/ entries\n${topLevel.join("\n")}\n\n## Tools (${tools.length})\n${tools.map((t) => `- ${t.name}: ${t.files.join(", ")}`).join("\n")}\n\n## Commands (${commands.length})\n${commands.map((c) => `- ${c.name} ${c.isDirectory ? "(dir)" : "(file)"}`).join("\n")}`,
                },
              },
            ],
          };
            ...
            messages: [
              {
                ...
                content: {
                  type: "text" as const,
                  text: `Compare these two Code Engine tools:\n\n## ${tool1}\n\`\`\`typescript\n${sources[0]}\n\`\`\`\n\n## ${tool2}\n\`\`\`typescript\n${sources[1]}\n\`\`\``,
                },
              },
            ],
          };
...
export async function validateSrcRoot(): Promise<void> {
  if (!(await dirExists(SRC_ROOT))) {
    console.error(
      `Error: Code Engine src/ directory not found at ${SRC_ROOT}`
    );
    console.error(
